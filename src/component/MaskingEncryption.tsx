  ...
  const [systems, setSystems] = useState<System[]>([
    {
      ...
      metrics: {
        encrypted: { value: '94%', label: 'Encrypted Data', color: 'bg-primary-soft' },
        masked: { value: '3', label: 'Masked Fields', color: 'bg-success-soft' },
        unencrypted: { value: '6', label: 'Unencrypted Exceptions', color: 'bg-surface-bg' }
      },
      securityStatus: {
    ...
    },
    {
      ...
      metrics: {
        encrypted: { value: '98%', label: 'Encrypted Data', color: 'bg-primary-soft' },
        masked: { value: '4', label: 'Masked Fields', color: 'bg-success-soft' },
        unencrypted: { value: '2', label: 'Unencrypted Exceptions', color: 'bg-surface-bg' }
      },
      securityStatus: {
    ...
    },
    {
      ...
      metrics: {
        encrypted: { value: '78%', label: 'Encrypted Data', color: 'bg-primary-soft' },
        masked: { value: '4', label: 'Masked Fields', color: 'bg-success-soft' },
        unencrypted: { value: '8', label: 'Unencrypted Exceptions', color: 'bg-surface-bg' }
      },
      securityStatus: {
    ...
    },
    {
      ...
      metrics: {
        encrypted: { value: '92%', label: 'Encrypted Data', color: 'bg-primary-soft' },
        masked: { value: '3', label: 'Masked Fields', color: 'bg-success-soft' },
        unencrypted: { value: '5', label: 'Unencrypted Exceptions', color: 'bg-surface-bg' }
      },
      securityStatus: {
        database: { status: 'Unmasked', badge: 'Unmasked' },
        storage: { status: 'Encrypted', badge: 'Encrypted' },
    ...
    <div className="min-h-screen bg-surface-bg">
      ...
      <div className="bg-white border-b border-primary/[0.06] sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <h1 className="text-xl sm:text-2xl font-bold text-txt-primary">Data Masking & Encryption Status</h1>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-4 sm:space-y-6">
        {systems.map((system) => (
          <div key={system.id} className="glass-panel overflow-hidden shadow-sm">
            <div className="p-4 sm:p-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      ...
                      {system.tags.map((tag, idx) => (
                        <span key={idx} className="px-2.5 py-0.5 bg-gray-100 text-txt-primary rounded text-xs flex-shrink-0">
                          {tag}
                        </span>
                      ))}
                    </div>
                    <div className="text-xs text-txt-muted break-words">{system.date}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div className={`p-3 sm:p-4 rounded-lg text-center ${system.metrics.encrypted.color}`}>
                  ...
                  <div className="text-xs text-txt-secondary mt-1">{system.metrics.encrypted.label}</div>
                </div>
                <div className={`p-3 sm:p-4 rounded-lg text-center ${system.metrics.masked.color}`}>
                  <div className="text-2xl sm:text-3xl font-bold text-success">
                    {system.metrics.masked.value}
                  </div>
                  <div className="text-xs text-txt-secondary mt-1">{system.metrics.masked.label}</div>
                </div>
                <div className={`p-3 sm:p-4 rounded-lg text-center ${system.metrics.unencrypted.color}`}>
                  ...
                  <div className="text-xs text-txt-secondary mt-1">{system.metrics.unencrypted.label}</div>
                </div>
              </div>

              {/* Security Status */}
              <div className="mb-4">
                <h3 className="text-xs sm:text-sm font-semibold text-txt-primary mb-3">Security Status:</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <span className="text-xs text-txt-secondary">Database</span>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-txt-primary">{system.securityStatus.database.status}</span>
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${getStatusClass(system.securityStatus.database.badge)}`}>
                        {system.securityStatus.database.badge}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <span className="text-xs text-txt-secondary">Storage</span>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-txt-primary">{system.securityStatus.storage.status}</span>
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${getStatusClass(system.securityStatus.storage.badge)}`}>
                        {system.securityStatus.storage.badge}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <span className="text-xs text-txt-secondary">API</span>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-txt-primary">{system.securityStatus.api.status}</span>
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${getStatusClass(system.securityStatus.api.badge)}`}>
                        {system.securityStatus.api.badge}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <span className="text-xs text-txt-secondary">Backup</span>
                    <span className="text-xs text-txt-primary">{system.securityStatus.backup.status}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Security Controls */}
              <div className="mb-4">
                <h3 className="text-xs sm:text-sm font-semibold text-txt-primary mb-2">Security Controls:</h3>
              ...
              {/* Key Management */}
              <div className="flex items-start gap-3 p-3 sm:p-4 bg-surface-bg rounded-lg">
                <Shield className="text-txt-muted flex-shrink-0 mt-1" size={20} />
                <div className="flex-1 min-w-0">
                  <h3 className="text-xs font-semibold text-txt-primary mb-1">{system.keyManagement.method}</h3>
              ...
              {system.notice && (
                <div className={`mt-4 p-3 sm:p-4 rounded-lg ${
                  system.notice.type === 'error' ? 'bg-critical-soft border border-red-200' : 'bg-warning-soft border border-orange-200'
                }`}>
                  <p className={`text-xs ${
                    system.notice.type === 'error' ? 'text-critical' : 'text-amber-700'
                  } break-words`}>
