  ...
  return (
    <div className="min-h-screen bg-white p-3 sm:p-4 lg:p-6 xl:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Matrix Container */}
        <div className="bg-white border border-primary/[0.06] rounded-lg p-6 mb-6 overflow-x-auto">
            ...
            <table className="border-collapse">
              <thead>
                <tr>
                  ...
                  {likelihood.map((item, index) => (
                    <th key={index} className="px-4 py-3 text-sm font-medium text-txt-primary text-center min-w-48">
                      {item}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {impact.map((impactItem, rowIndex) => (
                  <tr key={rowIndex}>
                    <td className="px-4 py-3 text-sm font-medium text-txt-primary text-right pr-6 whitespace-nowrap">
                      {impactItem}
                    </td>
                    {matrix[rowIndex].map((cell, colIndex) => (
                      <td key={colIndex} className="p-2">
                        <div className={`${cell.color} h-20 flex items-center justify-center rounded-lg text-txt-primary font-semibold text-lg border border-primary/[0.08] hover:shadow-md transition-shadow cursor-pointer`}>
                          {cell.value}
                        </div>
                      </td>
                    ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {legends.map((legend, index) => (
            <div key={index} className="bg-white border border-primary/[0.06] rounded-lg p-4">
              <div className="flex items-center gap-3 mb-2">
                <div className={`w-6 h-6 rounded ${legend.color} border border-primary/[0.08]`}></div>
                <h3 className="font-semibold text-txt-primary text-sm">{legend.label}</h3>
              </div>
