  ...
  return (
    <div className="min-h-screen bg-surface-bg p-4 md:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          <h1 className="text-2xl md:section-header-title">Role-Based Access Settings</h1>
          <p className="text-sm text-txt-secondary mt-1">Configure feature access and permissions by role</p>
        </div>
      ...
      <div className="glass-panel mb-6">
            ...
            <tbody className="bg-white divide-y divide-primary/[0.06]">
              {filteredSettings.length === 0 ? (
                ...
                filteredSettings.map((setting) => (
                  <tr key={setting.id} className="hover:bg-surface-bg transition-colors">
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-txt-primary">{setting.feature}</div>
                      <div className="text-sm text-txt-muted">{setting.description}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-txt-primary">{setting.module}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-txt-primary">{setting.roleCount} roles</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${setting.status === 'Enabled' ? 'bg-success-soft0' : 'bg-critical'}`}></div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-txt-primary">{setting.lastModified}</div>
                      <div className="text-sm text-txt-muted">by {setting.modifiedBy}</div>
                    </td>
        ...
        <div className="lg:hidden divide-y divide-primary/[0.06]">
          {filteredSettings.length === 0 ? (
            ...
            filteredSettings.map((setting) => (
              <div key={setting.id} className="p-4 hover:bg-surface-bg">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <div className="text-sm font-medium text-txt-primary">{setting.feature}</div>
                    <div className="text-xs text-txt-muted mt-1">{setting.description}</div>
                  </div>
                ...
                <div className="space-y-2 mb-3">
                  <div className="text-sm">
                    ...
                    <span className="text-txt-primary">{setting.module}</span>
                  </div>
                  <div className="text-sm">
                  ...
                  <div className="text-sm">
                    ...
                    <span className="text-txt-primary">{setting.lastModified} by {setting.modifiedBy}</span>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="glass-panel p-6">
          <div className="text-sm text-txt-secondary mb-2">Feature Control</div>
          <div className="section-header-title mb-3">{stats.totalFeatures}</div>
          <div className="space-y-1">
            <div className="flex items-center justify-between text-sm">
              <span className="text-txt-secondary">Enabled</span>
              <span className="font-medium text-txt-primary">{stats.enabledFeatures}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-txt-secondary">Disabled</span>
              <span className="font-medium text-txt-primary">{stats.disabledFeatures}</span>
            </div>
          </div>
        </div>
        ...
        {/* Role Coverage Card */}
        <div className="glass-panel p-6">
          <div className="text-sm text-txt-secondary mb-2">Role Coverage</div>
          <div className="section-header-title mb-3">{stats.roleCoverage}%</div>
          <div className="text-sm text-txt-secondary mb-2">Roles have defined access</div>
          <div className="progress-track overflow-hidden" role="progressbar" aria-valuemin={0} aria-valuemax={100}>
            <div className="h-full bg-primary" style={{ width: `${stats.roleCoverage}%` }}></div>
          </div>
        </div>

        {/* Security Score Card */}
        <div className="glass-panel p-6">
          <div className="text-sm text-txt-secondary mb-2">Security Score</div>
          <div className="section-header-title mb-3">{stats.securityScore}/{stats.maxScore}</div>
          <div className="text-sm text-txt-secondary mb-2">Access control maturity</div>
          <div className="progress-track overflow-hidden" role="progressbar" aria-valuemin={0} aria-valuemax={100}>
            <div className="h-full bg-primary" style={{ width: `${(stats.securityScore / stats.maxScore) * 100}%` }}></div>
          </div>
        </div>
      </div>
    </div>
