    ...
    <div className="min-h-screen bg-surface-bg">
      ...
      <div className="bg-white border-b border-primary/[0.06]">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-txt-primary">Security Audit Tracker</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        {audits.map((audit) => (
          <div key={audit.id} className="glass-panel overflow-hidden">
            {/* Audit Header */}
            <div className="p-6">
                ...
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h2 className="text-lg font-semibold text-txt-primary">{audit.vendor}</h2>
                      <span className="px-2.5 py-0.5 bg-primary text-white rounded text-xs font-medium">
                        {audit.auditType}
                      </span>
                    </div>
              ...
              <div className="grid grid-cols-4 gap-6 mb-6">
                <div className="text-center p-4 rounded-lg" style={{ backgroundColor: '#ffe5e5' }}>
                  ...
                  <div className="text-3xl font-bold text-critical">{audit.critical}</div>
                  <div className="text-xs text-txt-secondary mt-1">Critical</div>
                </div>
                <div className="text-center p-4 rounded-lg" style={{ backgroundColor: '#fff4e5' }}>
                  <div className="flex items-center justify-center mb-2">
                    <AlertCircle className="text-orange-500" size={20} />
                  </div>
                  <div className="text-3xl font-bold text-amber-600">{audit.high}</div>
                  <div className="text-xs text-txt-secondary mt-1">High</div>
                </div>
                <div className="text-center p-4 rounded-lg" style={{ backgroundColor: '#fffbeb' }}>
                  <div className="flex items-center justify-center mb-2">
                    <AlertCircle className="text-yellow-500" size={20} />
                  </div>
                  <div className="text-3xl font-bold text-amber-600">{audit.medium}</div>
                  <div className="text-xs text-txt-secondary mt-1">Medium</div>
                </div>
                <div className="text-center p-4 rounded-lg" style={{ backgroundColor: '#e8f4fd' }}>
                  ...
                  <div className="text-xs text-txt-secondary mt-1">Compliance</div>
                </div>
              </div>

              {/* Findings */}
              <div className="space-y-3 mb-6">
                <h3 className="text-sm font-semibold text-txt-primary">Key Findings:</h3>
                {audit.findings.map((finding, idx) => (
                  <div key={idx} className="flex items-start justify-between py-2.5 border-b border-gray-100 last:border-0">
                    <div className="flex-1">
                      <div className="text-sm text-txt-primary mb-1">{finding.description}</div>
                      <div className="text-xs text-txt-muted">{finding.recommendation}</div>
                    </div>
                    <div className="flex items-center gap-3 ml-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getSeverityClass(finding.status)}`}>
                        {finding.status}
                      </span>
                      {finding.assignee && (
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusClass(finding.assignee)}`}>
                          {finding.assignee}
                        </span>
                      )}
                    </div>
              ...
              <div className="pt-4 border-t border-primary/[0.06]">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-6 text-txt-secondary">
                    <span>Overall Rating: <span className="font-medium text-txt-primary">{audit.overallRating}</span></span>
                    <span>{audit.nextAudit}</span>
                  </div>
                  {audit.certificationDetails && (
                    <div className="text-txt-muted">{audit.certificationDetails}</div>
                  )}
                </div>
              </div>
            </div>
