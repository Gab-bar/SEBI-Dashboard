    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="border-b border-primary/[0.06]">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-txt-primary">Risk Treatment Plans</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        {plans.map((plan) => (
          <div key={plan.id} className="border border-primary/[0.06] rounded-lg overflow-hidden">
            {/* Plan Header */}
            <div className="bg-surface-bg p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1">
                    <h2 className="text-lg font-semibold text-txt-primary mb-1">{plan.title}</h2>
              ...
              <div className="grid grid-cols-4 gap-8">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs text-txt-muted mt-1">Likelihood</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl font-bold text-blue-500">{plan.riskScore}</div>
                  <div className="text-xs text-txt-muted mt-1">Risk Score</div>
                </div>
                <div className="text-center">
                  <div className="text-xl font-bold text-purple-600">{plan.budget}</div>
                  <div className="text-xs text-txt-muted mt-1">Total Budget</div>
                </div>
              </div>

              <div className="mt-4 text-right text-xs text-txt-muted">
                {plan.targetDate}
              </div>
            ...
            {plan.isExpanded && (
              <div className="bg-surface-bg border-t border-primary/[0.06]">
                <div className="p-6">
                  <h3 className="text-sm font-semibold text-txt-primary mb-4">Treatment Activities</h3>
                  <div className="space-y-3">
                    {plan.treatments.map((treatment, idx) => (
                      <div key={idx} className="bg-white rounded-lg p-4 border border-primary/[0.06]">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-start gap-3">
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-txt-muted">{treatment.dueDate}</p>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            ...
                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusClass(treatment.status)}`}>
                              {treatment.status}
                            </span>
                          </div>
                        </div>
