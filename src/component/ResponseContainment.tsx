  ...
  return (
    <div className="min-h-screen bg-white p-8">
      <div className="max-w-5xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-txt-primary mb-4">Incident Response Timeline</h1>
          <div className="flex items-center gap-3">
            <span className="text-base font-medium text-txt-primary">Incident: {incidentId}</span>
              ...
              <div key={index} className="relative pl-8">
                ...
                <div className={`absolute left-0 top-0 w-4 h-4 rounded-full border-2 ${
                  event.status === 'Completed' 
                    ? 'bg-success-soft0 border-green-500' 
                    : 'bg-warning-soft0 border-yellow-500'
                }`} />

                {/* Content */}
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-base font-semibold text-txt-primary">{event.title}</h3>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      event.status === 'Completed' 
                        ? 'bg-primary text-white' 
                        : 'bg-gray-100 text-txt-primary border border-primary/[0.08]'
                    }`}>
                      {event.status}
                    </span>
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded text-xs bg-white text-txt-primary border border-primary/[0.08]">
                      {event.category}
                    </span>
                  </div>

                  <p className="text-sm text-txt-primary mb-2">{event.description}</p>
