    ...
    <div className="bg-surface-bg min-h-screen p-3 sm:p-4 lg:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-txt-primary">Committee Members Directory</h2>
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 border border-primary/[0.08] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 w-64"
              />
            </div>
            <select
              value={statusFilter}
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="bg-gray-100 rounded-full p-4 flex-shrink-0">
                  <User className="w-6 h-6 text-txt-secondary" />
                </div>
                ...
                <div className="flex-1 min-w-0">
                      ...
                      <span
                        className={`text-xs font-medium px-3 py-1 rounded-full ${
                          member.status === 'Active'
                            ...
                            : 'bg-gray-200 text-txt-secondary'
                        }`}
                      >
                        {member.status}
                      </span>
                  ...
                  <p className="text-base font-semibold text-txt-primary mb-1">{member.title}</p>
                  <p className="text-sm text-txt-secondary mb-4">{member.department}</p>
                  ...
                  {/* Expertise */}
                  <div className="mb-4">
                    <p className="text-sm font-semibold text-txt-primary mb-2">Expertise:</p>
                    <div className="flex flex-wrap gap-2">
                      {member.expertise.map((skill, index) => (
                        <span
                          key={index}
                          className="text-xs font-medium px-3 py-1 bg-white border border-primary/[0.08] rounded-full text-txt-primary"
                        >
                          {skill}
                        </span>
                      ))}
                  ...
                  <div className="flex items-center gap-6 text-sm">
                    <div>
                      <span className="font-medium text-txt-primary">Experience:</span>{' '}
                      <span className="text-txt-secondary">{member.experience}</span>
                    </div>
                    <div>
                      <span className="font-medium text-txt-primary">Attendance:</span>{' '}
                      <span className="text-txt-secondary">{member.attendance}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />
                      <span className="font-medium text-txt-primary">Score:</span>
                      <span className="text-txt-secondary">{member.score}/5</span>
                    </div>
                  </div>
                </div>
              </div>
