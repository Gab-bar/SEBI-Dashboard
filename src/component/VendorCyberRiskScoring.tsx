    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="border-b border-primary/[0.06]">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <h1 className="text-2xl font-bold text-txt-primary">Vendor Cyber Risk Scoring Matrix</h1>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
          ...
          {riskCategories.map((category, idx) => (
            <div key={idx} className="glass-panel p-5">
              <div className="flex items-start justify-between mb-3">
                <h3 className="text-sm font-semibold text-txt-primary">{category.title}</h3>
                <button className="text-txt-muted hover:text-txt-secondary">
                  <Info size={16} />
                </button>
              </div>
              <p className="text-xs text-txt-muted mb-4">{category.description}</p>
              
              <div className="space-y-2">
                {category.items.map((item, itemIdx) => (
                  <div key={itemIdx} className="flex items-center justify-between py-1.5 border-b border-gray-100 last:border-0">
                    <span className="text-xs text-txt-primary">{item.name}</span>
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                        item.weight >= 4 ? 'bg-critical-soft text-critical' : 
                        item.weight >= 3 ? 'bg-warning-soft text-amber-700' : 
        ...
        <div className="bg-surface-bg rounded-lg border border-primary/[0.06] p-5 mb-8">
          ...
          <div className="grid grid-cols-4 gap-4 text-xs">
            ...
            <div>
              <span className="text-txt-muted">Medium Risk:</span>
              <span className="ml-2 font-medium text-txt-primary">60-79</span>
            </div>
            <div>
              <span className="text-txt-muted">High Risk:</span>
              <span className="ml-2 font-medium text-txt-primary">0-59</span>
            </div>
            <div>
              <span className="text-txt-muted">Critical Risk:</span>
              <span className="ml-2 font-medium text-txt-primary">0-40</span>
            </div>
          </div>
        </div>
        ...
        <div>
          <h2 className="text-lg font-semibold text-txt-primary mb-4">Current Vendor Risk Scores</h2>
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-primary/[0.06]">
                {currentVendors.map((vendor) => (
                  <tr key={vendor.id} className="hover:bg-surface-bg transition-colors">
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-txt-primary">{vendor.name}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-txt-secondary">{vendor.category}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getRiskClass(vendor.riskLevel)}`}>
                        {vendor.riskLevel}
                      </span>
                    </td>
                  </tr>
