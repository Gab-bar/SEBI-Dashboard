...
const TrainingMaterialsRepository: React.FC = () => {
  ...
  const getAccessColor = (access: string) => {
    switch (access) {
      ...
      case 'Restricted':
        return 'text-amber-600 bg-warning-soft';
      case 'Confidential':
        return 'text-critical bg-critical-soft';
      default:
        return 'text-txt-secondary bg-surface-bg';
    }
  };
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1 sm:w-80">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-primary/[0.08] rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-ring focus:border-transparent text-sm"
                />
              </div>
              <button className="bg-primary text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-primary-hover transition-colors flex items-center justify-center gap-2 whitespace-nowrap">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 md:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          {trainingMaterials.map((material) => (
            <div
              key={material.id}
              className="glass-panel border border-primary/[0.06] hover:shadow-md transition-all overflow-hidden"
            >
              <div className="p-4 md:p-6">
                {/* Title and Badge */}
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2 flex-wrap flex-1">
                    <h2 className="text-base md:text-lg font-semibold text-txt-primary">
                      {material.title}
                    </h2>
                    {material.badge && (
                      <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusClass(material.badge)}`}>
                        {material.badge}
                      </span>
                    )}
                  </div>
                  <button className="text-txt-muted hover:text-txt-secondary p-1">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 font-medium text-txt-primary">{material.version}</span>
                  </div>
                  <div>
                    <span className="text-txt-muted">Size:</span>
                    <span className="ml-2 font-medium text-txt-primary">{material.size}</span>
                  </div>
                  <div>
                    <span className="text-txt-muted">Downloads:</span>
                    <span className="ml-2 font-medium text-txt-primary">{material.downloads.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                    <span className="font-medium text-txt-primary">{material.rating}/5</span>
                  </div>
                </div>

                {/* Topics */}
                <div className="mb-4">
                  <p className="text-xs text-txt-muted mb-2 font-medium">Topics:</p>
                  <div className="flex flex-wrap gap-2">
                    {material.topics.map((topic, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-1 bg-gray-100 text-txt-primary rounded text-xs hover:bg-gray-200 transition-colors"
                      >
                        {topic}
                      </span>
                    ))}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-4 border-t border-gray-100">
                  ...
                  <button className="bg-primary text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-primary-hover transition-colors w-full sm:w-auto">
                    Download
                  </button>
                </div>
              </div>
        ...
        <div className="mt-8 text-center">
          <button className="border border-primary/[0.08] text-txt-primary px-6 py-2 rounded-lg text-sm font-medium hover:bg-surface-bg transition-colors">
            Load More Materials
          </button>
        </div>
      </main>
