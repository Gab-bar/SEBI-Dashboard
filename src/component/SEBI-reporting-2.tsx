    ...
    <div className="min-h-screen bg-surface-bg">
      ...
      <div className="bg-white border-b border-primary/[0.06]">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-txt-primary">GRC Risk Reporting</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
        {reports.map((report) => (
          <div key={report.id} className="glass-panel overflow-hidden">
                ...
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h2 className="text-lg font-semibold text-txt-primary">{report.title}</h2>
                      <span className="px-3 py-1 bg-gray-200 text-txt-primary rounded text-xs font-medium">
                        {report.quarter}
                      </span>
                      <span className="px-3 py-1 bg-gray-200 text-txt-primary rounded text-xs font-medium">
                        {report.high}
                    ...
                    <div className="flex flex-wrap gap-4 text-xs text-txt-secondary">
                ...
                {report.items.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between py-2">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      {item.details && (
                        <>
                          <span className={`px-2 py-0.5 rounded text-xs font-medium ${getStatusClass(item.statusType ?? '')}`}>
                            {item.status}
                          </span>
                          <span className="text-xs text-txt-muted">{item.details}</span>
                        </>
                      )}
                    </div>
                    {item.statusType && !item.details && (
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusClass(item.statusType)}`}>
                        {item.status}
                      </span>
                    )}
                  </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 pt-6 border-t border-primary/[0.06]">
                {report.metrics.map((metric, idx) => (
                  <div key={idx} className="text-center">
                    ...
                    <div className="text-xs text-txt-muted mt-1">{metric.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Expanded Content */}
            {report.isExpanded && (
              <div className="bg-surface-bg border-t border-primary/[0.06] p-6">
                <h3 className="text-sm font-semibold text-txt-primary mb-4">Risk Assessment Details</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white p-4 rounded border border-primary/[0.06]">
                    <div className="text-xs text-txt-muted mb-1">Assessment Period</div>
                    <div className="text-sm font-medium text-txt-primary">Q4 2024</div>
                  </div>
                  <div className="bg-white p-4 rounded border border-primary/[0.06]">
                    <div className="text-xs text-txt-muted mb-1">Next Review</div>
                    <div className="text-sm font-medium text-txt-primary">2025-04-15</div>
                  </div>
                  <div className="bg-white p-4 rounded border border-primary/[0.06]">
                    <div className="text-xs text-txt-muted mb-1">Risk Trend</div>
                    <div className="text-sm font-medium text-success">↓ Decreasing</div>
                  </div>
                  <div className="bg-white p-4 rounded border border-primary/[0.06]">
                    <div className="text-xs text-txt-muted mb-1">Overall Status</div>
                    <div className="text-sm font-medium text-primary">On Track</div>
                  </div>
                </div>
              </div>
            )}
