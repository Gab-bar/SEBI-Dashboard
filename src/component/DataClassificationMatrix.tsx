...
const DataClassificationMatrix = () => {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'Internal':
        return 'bg-primary-soft0 text-white';
      case 'Public':
      ...
      <div className="bg-white border-b border-primary/[0.06]">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-bold text-txt-primary">Data Classification Matrix</h1>
          ...
          <div key={item.id} className="glass-panel overflow-hidden">
            <div className="p-6">
                ...
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h2 className="text-lg font-semibold text-txt-primary">{item.title}</h2>
              ...
              <div className="grid grid-cols-3 gap-6 mb-4">
                {/* Storage & Access */}
                <div>
                  <h3 className="text-xs font-semibold text-txt-primary mb-2">{item.storageAccess.storage}</h3>
                  <div className="text-xs text-txt-secondary space-y-1">
                    <p>{item.storageAccess.access}</p>
                    <p className="text-txt-muted">{item.storageAccess.retention}</p>
                  </div>
                </div>

                {/* Security Controls */}
                <div>
                  <h3 className="text-xs font-semibold text-txt-primary mb-2">{item.securityControls.encryption}</h3>
                  <div className="text-xs text-txt-secondary space-y-1">
                    <p>{item.securityControls.accessControl}</p>
                    <p className="text-txt-muted">{item.securityControls.monitoring}</p>
                  </div>
                </div>

                {/* Compliance */}
                <div>
                  <h3 className="text-xs font-semibold text-txt-primary mb-2">{item.compliance.framework}</h3>
                  <div className="text-xs text-txt-secondary space-y-1">
                    <p>{item.compliance.status}</p>
                    <p className="text-txt-muted">{item.compliance.auditFrequency}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4">
                <h3 className="text-xs font-semibold text-txt-primary mb-2">Regulatory Requirements</h3>
              ...
              {item.isExpanded && (
                <div className="pt-4 border-t border-primary/[0.06]">
                  <h3 className="text-xs font-semibold text-txt-primary mb-2">Business Justification</h3>
                  <p className="text-xs text-txt-secondary">{item.businessJustification}</p>
                </div>
              )}
            </div>
          </div>
