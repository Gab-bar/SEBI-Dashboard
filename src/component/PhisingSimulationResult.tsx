...
const PhishingSimulationResults: React.FC = () => {
  const getPercentageColor = (percentage: number): string => {
    if (percentage >= 80) return 'bg-success-soft0';
    if (percentage >= 60) return 'bg-warning-soft0';
    return 'bg-critical';
  };
    ...
    <div className="min-h-screen bg-surface-bg">
      ...
      <header className="bg-white border-b border-primary/[0.06] px-4 py-4 md:px-6 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button className="bg-primary text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-primary-hover transition-colors whitespace-nowrap">
            New Campaign
          </button>
        </div>
      </header>
            ...
            <div className="p-4 md:p-6 border-b border-gray-100">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-3">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-sm text-txt-secondary">{campaign.status}</span>
                </div>
                <button className="text-txt-muted hover:text-txt-secondary self-end md:self-auto">
                  <ChevronDown className="w-5 h-5" />
                </button>
              </div>
              <p className="text-sm text-txt-secondary">{campaign.duration}</p>
            </div>

            {/* Stats Grid */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 md:p-6 bg-surface-bg">
              <div className="bg-primary-soft rounded-lg p-4 text-center">
                <div className="text-3xl md:text-4xl font-bold text-primary mb-1">
                  {campaign.totalSent}
                </div>
                <div className="text-xs text-txt-secondary">Email Sent</div>
              </div>
              <div className="bg-warning-soft rounded-lg p-4 text-center">
                <div className="text-3xl md:text-4xl font-bold text-amber-600 mb-1">
                  {campaign.clickRate}
                </div>
                <div className="text-xs text-txt-secondary">Click Rate</div>
              </div>
              <div className="bg-success-soft rounded-lg p-4 text-center">
                <div className="text-3xl md:text-4xl font-bold text-success mb-1">
                  {campaign.reportRate}
                </div>
                <div className="text-xs text-txt-secondary">Report Rate</div>
              </div>
              <div className="bg-purple-50 rounded-lg p-4 text-center col-span-2 md:col-span-1">
                ...
                <div className="text-xs text-txt-secondary">Pass Rate</div>
              </div>
            </div>

            {/* Department Performance */}
            <div className="p-4 md:p-6">
              <h3 className="text-base md:text-lg font-semibold text-txt-primary mb-4">
                Department Performance:
              </h3>
              ...
              <div className="space-y-3">
                {campaign.departments.map((dept, idx) => (
                  <div key={idx} className="flex flex-col space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-baseline gap-2 mb-1">
                          <span className="font-medium text-txt-primary text-sm md:text-base truncate">
                            {dept.name}
                          </span>
                      ...
                      <div className="flex items-center gap-3 ml-4">
                        <span className="text-xs text-txt-secondary whitespace-nowrap hidden sm:inline">
                    ...
                    {/* Mobile stats */}
                    <div className="text-xs text-txt-secondary sm:hidden">
                      Clicked: {dept.clicked} | Reported: {dept.reported}
                    </div>
